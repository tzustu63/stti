    ...
    try {
      ...
      switch (message.type) {
        case 'start_recording':
          await handleStartRecording(clientId, message);
          break;
...
async function handleStartRecording(clientId, message) {
  try {
    ...
    // 建立 GLADIA 會話
    const gladiaSessionId = await gladiaService.createSession(session.inputLanguage);
...
async function handleAudioData(clientId, message) {
  try {
    ...
    // 發送到 GLADIA
    const gladiaResponse = await gladiaService.sendAudioData(session.gladiaSessionId, audioBuffer);
    ...
    if (asrResult && asrResult.text) {
      ...
      if (asrResult.isFinal) {
        await translateText(clientId, asrResult.text, session.inputLanguage, session.outputLanguage);
      }
    }
...
async function translateText(clientId, text, sourceLang, targetLang) {
  try {
    ...
    const translationResult = await deeplService.translate(text, targetLang, sourceLang);
